...
final class FollowViewReactor: Reactor {
    ...
    enum Mutation {
        ...
        case fetchFollow([FollowUser], nextPage: Int, canLoadMore: Bool)
        case fetchMoreFollow([FollowUser], nextPage: Int, canLoadMore: Bool)
        case setPage(Int)
    ...
    func mutate(action: Action) -> Observable<Mutation> {
        ...
        case .switchSegmentControl:
            ...
            let clearPagingMutation = self.clearPaging()
            let trendMutation: Observable<Mutation> = .just(.setFollowType(followType))
            let requestMutation: Observable<Mutation> = self.requestFollow(followType: followType)
            return .concat([clearPagingMutation, trendMutation, requestMutation])
        case .loadMore:
            guard !self.currentState.isRefreshing else { return .empty() }
            guard !self.currentState.isLoading else { return .empty() }
